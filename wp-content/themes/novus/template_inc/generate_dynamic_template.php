...
class GenerateDynamicTemplate{
    ...
    function GenerateDynamicTemplate($template = false){
        global $themeple_controller, $post;
        if($template != false){
            $ar = explode("-", $template);
            $this->temp_name = $ar[1];
    ...
    function getTemplateElements(){
        ...
        if(isset($themeple_controller->options['builder']))
            $themeple_controller->options['builder'] = themeple_entity_decode($themeple_controller->options['builder']);
        
        foreach($themeple_controller->page_elements as $key => $element){
            
            if('dynamic_template-'.$element['slug'] == $this->template_type && isset($element['dynamic'])){
                
                if(isset($themeple_controller->options['builder'][$element['id']])){
                    $themeple_controller->page_elements[$key]['saved'] = $themeple_controller->options['builder'][$element['id']];
    ...
    function getTemplateOption($option_id){
        ...
        $option = false;
        if(isset($themeple_controller->options['builder']) && isset($themeple_controller->options['builder'][$this->temp_name.$option_id])){
            $option = themeple_entity_decode($themeple_controller->options['builder'][$this->temp_name.$option_id]);
        }
    ...
    function sticky_box($element){
        ...
        $output = '<div class="span'.$dynamic_size.' sticky_box">';
            $output .= '<div class="stickyy">';
                ...
                //$new_color = colourBrightness(themeple_get_option('base_color'), 0.4);

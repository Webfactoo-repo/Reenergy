<?php   
        ...
        if($dynamic_content_type == 'content'){
            ...
            if($icon_bool == 'yes' && $icon_bool_pred == 'yes' && !empty($icon)):
                ...
                $output .= '<div class="icon_wrapper"><div class="overlay"></div><i class="'.$icon.'"></i></div>';
            ...
            if($icon_bool == 'yes'  && $icon_bool_pred == 'no' && !empty($icon_up)):
               ...
               $image_attributes = wp_get_attachment_image_src( $icon_up );
                $output .= '<span class="icon_up" style="background:url(\''. $image_attributes[0].'\') center no-repeat;"></span>';
